...
export async function generateStaticParams() {
  ...
  const remote = (data ?? []).map((r) => ({ slug: r.slug as string }));
  const local = BLOGS_FALLBACK.map((b) => ({ slug: b.slug }));
  const map = new Map<string, { slug: string }>();
  [...remote, ...local].forEach((x) => map.set(x.slug, x));
...
export default async function BlogDetailPage({
  ...
  const blog =
    ...
    (() => {
      const local = BLOGS_FALLBACK.find((b) => b.slug === slug);
        ...
        <main className="min-h-screen bg-[#f6f4ef] text-[#0c1b2a] pt-28 sm:pt-32 pb-20">
          <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="mt-8 flex justify-center">
              <Link
                href="/blogs"
                className="inline-flex min-h-[48px] items-center justify-center rounded-xl bg-gradient-to-r from-[#C5A24A] to-[#EBD181] px-8 py-3 text-base font-bold text-[#001F3F] shadow-md transition hover:brightness-105 active:scale-[0.98]"
              >
                Back to Blogs
              </Link>
            </div>
    ...
    <>
      ...
      <main className="min-h-screen bg-[#f6f4ef] text-[#0c1b2a] pt-28 sm:pt-32 pb-0">
        <div className="relative overflow-hidden bg-[#0c1b2a] pb-8 pt-6 sm:pb-12 sm:pt-8">
          ...
          />
          <div
            className="pointer-events-none absolute left-10 top-16 hidden sm:block animate-float-slow"
            aria-hidden
          >
            <div className="rounded-2xl border border-white/10 bg-white/10 p-3 backdrop-blur-md">
              <ArrowLeft className="h-6 w-6 text-[#EBD181]" aria-hidden />
            </div>
          </div>
        ...
        <div className="relative z-20 mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 -mt-6 pb-20 sm:-mt-10 sm:pb-24">
          <article className="overflow-hidden rounded-3xl border border-[#C5A24A]/15 bg-white shadow-[0_30px_90px_-28px_rgba(12,27,42,0.18)]">
            <div className="relative aspect-[21/9] min-h-[200px] w-full bg-[#0c1b2a] sm:aspect-[21/8]">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={blog.cover_image || "/images/Hero/3.jpg"}
                alt={blog.title}
                className="h-full w-full object-cover"
              />
              <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#0c1b2a]/40 to-transparent" />
            </div>
            <div className="border-t border-[#C5A24A]/10 px-6 py-10 sm:px-12 sm:py-14">
              <div className="mx-auto max-w-3xl">
                <div className="mb-10 flex items-center gap-4">
                  <div className="h-px flex-1 bg-gradient-to-r from-transparent via-[#C5A24A]/40 to-transparent" />
                  <span className="shrink-0 text-xs font-semibold uppercase tracking-[0.2em] text-[#C5A24A]">
                    Reading
                  </span>
                  <div className="h-px flex-1 bg-gradient-to-r from-transparent via-[#C5A24A]/40 to-transparent" />
                </div>
                <div className="prose prose-lg prose-slate max-w-none prose-headings:font-semibold prose-headings:text-[#0c1b2a] prose-p:text-[#0c1b2a]/80 prose-p:leading-relaxed">
                  <div className="whitespace-pre-line text-[1.0625rem] leading-[1.75] text-[#0c1b2a]/82 sm:text-lg">
                    {blog.content}
                  </div>
                </div>
                <div className="mt-14 flex flex-col items-center justify-between gap-6 border-t border-[#0c1b2a]/8 pt-10 sm:flex-row">
                  ...
                  <Link
                    href="/contact"
                    className="inline-flex min-h-[48px] shrink-0 items-center justify-center rounded-xl bg-gradient-to-r from-[#C5A24A] to-[#EBD181] px-8 py-3 text-sm font-bold text-[#001F3F] shadow-md transition hover:brightness-105 active:scale-[0.98]"
                  >
